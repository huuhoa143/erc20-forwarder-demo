...
const gasMultiplier = ethers.BigNumber.from(2); //being safe
...
class TokenWallet{
    ...
    async estimateTransferGas(to,amount){
        return (ethers.BigNumber.from(50000)).mul(gasMultiplier);
    }

    //(await transferHandler.estimateGas.transfer(tokenAddress,to,amount)

    async estimateTransferCost(to,amount){
        return ((await this.estimateTransferGas(to,amount)).add(ethers.BigNumber.from(50000)).mul(ethers.BigNumber.from(await this.getTokenGasPrice())).mul(ethers.BigNumber.from(15))).div(ethers.BigNumber.from(10));
    }
